...
function updateNetworkStatus() {
  if ('connection' in navigator) {
    ...
    networkStatus.textContent = `Network: ${conn.effectiveType}, ${conn.downlink}Mbps`;
    conn.addEventListener('change', updateNetworkStatus);
  } else {
    networkStatus.textContent = `Network info unavailable`;
  }
}
updateNetworkStatus();
...
function drawRoute() {
  ...
  const pad = 20;
  function norm(val, min, max, size) {
